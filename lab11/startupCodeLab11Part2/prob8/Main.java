package lab11.startupCodeLab11Part2.prob8;
...
public class Main {
	...
	public static void main(String[] args) {
		...
		System.out.println("Do any of these Order Items have an order of flowers? " +
				m.findProductWithOptional("Flowers"));
	}

	private boolean findProductWithOptional(String prodName) {
		for (OrderItem item : orderItems) {
			Optional<String> opt = Optional.of(item)
					.map(p -> p.getProduct())
					.map(p -> p.getProductName())
					.filter(n -> n.equals(prodName));
